...
namespace Gatonini.Server.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class ModelsController : GenericController<Model>
    {
        private readonly IGenericRepositoryWrapper<Model> repositoryWrapper;
        private readonly IConfigSettings _settings;
        private readonly IMapper _mapper;
        public ModelsController(IGenericRepositoryWrapper<Model> wrapper,
        ...
        [HttpPatch("id")]
        public async Task<ActionResult<Model>> PatchUpdateAsync([FromBody] JsonPatchDocument value, [FromHeader] Guid id)
        {
            try
            {
        ...
        public async Task<ActionResult<Model>> Delete([FromRoute] Guid id)
        {
            try
            {
                ...
                var identity = await repositoryWrapper.Item.GetBy(x => x.ModelId.ToString().
                Equals(claim));
                if (identity.Count() != 0)
                {
                    Model u = new Model();
