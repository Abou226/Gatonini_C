...
namespace Gatonini.Server.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class TaillesController : GenericController<Taille>
    {
        private readonly IGenericRepositoryWrapper<Taille> repositoryWrapper;
        private readonly IConfigSettings _settings;
        private readonly IMapper _mapper;
        public TaillesController(IGenericRepositoryWrapper<Taille> wrapper,
        ...
        [HttpPatch("id")]
        public async Task<ActionResult<Taille>> PatchUpdateAsync([FromBody] JsonPatchDocument value, [FromHeader] Guid id)
        {
            try
            {
        ...
        public async Task<ActionResult<Categorie>> Delete([FromRoute] Guid id)
        {
            try
            {
                ...
                var identity = await repositoryWrapper.Item.GetBy(x => x.TailleId.ToString().
                Equals(claim));
                if (identity.Count() != 0)
                {
                    Taille u = new Taille();
