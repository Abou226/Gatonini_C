...
namespace Gatonini.Server.Controllers
{
    ...
    [ApiController]
    public class TaillesController : GenericController<Taille, TblUser>
    {
        private readonly IGenericRepositoryWrapper<Taille, TblUser> repositoryWrapper;
        public TaillesController(IGenericRepositoryWrapper<Taille, TblUser> wrapper) : base(wrapper)
        {
            repositoryWrapper = wrapper;
        }

        [HttpDelete("{id}")]
        public async Task<ActionResult<Taille>> Delete([FromRoute] Guid id)
        {
            try
            {
        ...
        public override async Task<ActionResult<IEnumerable<Taille>>> GetBy(string search)
        {
            try
            {
                ...
                if (identity.Count() != 0)
                {
                    var result = await repositoryWrapper.ItemA.GetBy(x => x.TailleId.ToString().Equals(search));
        ...
        [HttpPatch("{id:Guid}")]
        public async Task<ActionResult<Taille>> PatchUpdateAsync([FromBody] JsonPatchDocument value, [FromRoute] Guid id)
        {
            try
            {
                var item = await repositoryWrapper.Item.GetBy(x => x.TailleId == id);
                if (item.Count() != 0)
                {
