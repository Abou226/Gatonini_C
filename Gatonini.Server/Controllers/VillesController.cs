...
namespace Gatonini.Server.Controllers
{
    ...
    [ApiController]
    public class VillesController : GenericController<Ville, User>
    {
        private readonly IGenericRepositoryWrapper<Ville, User> repositoryWrapper;
        public VillesController(IGenericRepositoryWrapper<Ville, User> wrapper) : base(wrapper)
        {
        ...
        public override async Task<ActionResult<IEnumerable<Ville>>> GetBy(string search, DateTime start, DateTime end)
        {
            try
            {
                ...
                if (identity.Count() != 0)
                {
                    var result = await repositoryWrapper.ItemA.GetBy(x => x.Name.Contains(search) || x.Pays.Name.Contains(search));
