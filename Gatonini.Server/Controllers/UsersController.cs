    ...
    public class UsersController : GenericController<User>
    {
        ...
        public override async Task<ActionResult<IEnumerable<User>>> GetAll()
        {
            try
            {
                ...
                if (identity.Count() != 0)
                {
                    var result = await repositoryWrapper.Item.GetBy(x => (x.Id == identity.FirstOrDefault().Id));
        ...
        public override async Task<ActionResult<IEnumerable<User>>> GetBy(string search)
        {
            try
            {
                ...
                if (identity.Count() != 0)
                {
                    var result = await repositoryWrapper.Item.GetBy(x => x.Username.ToString().Equals(search)
                    || x.Nom.Contains(search) || x.Prenom.Contains(search));
